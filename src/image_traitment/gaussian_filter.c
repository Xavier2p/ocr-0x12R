...
void box_blur_T_4(Image *image_src, Image *image_dst, int w, int h, int r)
{
    ...
    for (int x = 0; x < w; x++)
    {
        ...
        int fv = src[ti][x].r, lv = src[ti + w * (h - 1)][x].r, val = (r + 1) * fv;
        for (int j = 0; j < r; j++)
            val += src[ti + j * w][x].r;
        for (int j = 0; j <= r; j++)
        {
            val += src[ri][x].r - fv;
            set_all_pixel(image_dst, ti, x, round(val * iarr));
            ri += w;
            ti += w;
        }
        for (int j = r + 1; j < w - r; j++)
        {
            val += src[ri][x].r - dst[li][x].r;
            set_all_pixel(image_dst, ti, x, round(val * iarr));
            li += w;
            ri += w;
            ti += w;
...
void box_blur_H_4(Image *image_src, Image *image_dst, int w, int h, int r)
{
    ...
    for (int y = 0; y < h; ++y)
    {
        ...
        int fv = src[y][ti].r, lv = src[y][ti + w - 1].r, val = (r + 1) * fv;
        for (int j = 0; j < r; ++j)
            val += src[y][ti + j].r;
        for (int j = 0; j <= r; ++j)
        {
            val += src[y][ri++].r - fv;
            set_all_pixel(image_dst, y, ti++, round(val * iarr));
        }
        for (int j = r + 1; j < w - r; ++j)
        {
            val += src[y][ri++].r - src[y][li++].r;
            set_all_pixel(image_dst, y, ti++, round(val * iarr));
        }
        for (int j = w - r; j < w; ++j)
        {
...
//}
//
//void gaussian_blur2(Image *image, int radius)
//{
//    int kwidht = (2 * round(radius)) + 1;
