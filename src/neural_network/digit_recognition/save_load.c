...
void save_weights(Network *network, char *path)
{
    ...
    if (network->nb_layers - 2 > 0)
        sprintf(str_header, "%d|%d", (int)(network->nb_layers - 2), network->layers[1].nb_neurons);
    else
    ...
    *network = new_network(784, nb_layers, nb_neurons, 10);
    ...
    while ((curr_char = getc(file)) != EOF)
    {
        switch (curr_char)
        {
            ...
            case '|':
                network->layers[layers_index].neurons[neurons_index].weights[weights_index] = atof(strtmp);
                memset(strtmp, 0, sizeof(strtmp));
                weights_index++;
                break;
